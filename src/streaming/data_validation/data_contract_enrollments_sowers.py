...
ENROLLMENT_REQUIRED_FIELDS: Final[list[str]] = [
    "order_id",
    "datetime",
    "region_id",
    "currency_code",
...
ALLOWED_DEVICE_TYPES: Final[set[str]] = {"mobile", "desktop", "tablet", "laptop"}
...
def validate_sale_record(
    *,
    ...
    errors.extend(
        validate_required_fields(
            record=record, required_fields=ENROLLMENT_REQUIRED_FIELDS
        )
    )
    ...
    if record.get("referral_source", "").strip():  # noqa: SIM102
...
def keep_sales_fields(row: dict[str, Any]) -> dict[str, Any]:
    ...
    """
    return {field: row.get(field, "") for field in ENROLLMENT_REQUIRED_FIELDS}
