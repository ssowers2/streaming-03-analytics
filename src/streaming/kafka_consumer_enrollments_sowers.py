...
from datetime import datetime  # Added import for technical modification
import os
...
OUTPUT_CSV: Final[Path] = OUTPUT_DIR / "consumed_enrollments_sowers.csv"
...
def process_message(
    ...
    *,
    ...
    errors = validate_required_fields(
        record=row, required_fields=ENROLLMENT_REQUIRED_FIELDS
    )
    if errors:
    ...
    enriched["discount_applied"] = bool(row.get("discount_code", "").strip())
...
def consume_messages(
    ...
    *,
    ...
    while consumed_count + skipped_count < MAX_MESSAGES:
        ...
        LOG.info(f"  Total enrollment revenue:  ${stats.total:,.2f}")
        LOG.info(f"  Average enrollment: ${stats.mean:,.2f}")
